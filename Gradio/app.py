...
class Predictor:
    ...
    def process_single(
        ...
        character_threshold: float,
        character_mcut: bool,
    ) -> tuple[str, list | dict | None, str | None, dict | None]:
        if image is None:
            return "Please provide an image.", None, None, None
        ...
        if process_type == "tag":
            return str(result.body), None, None, None
        if process_type in {"arrary", "array"}:
            return "", result.body if isinstance(result.body, list) else [], None, None
        if process_type == "tagimg":
            ...
            return "", {"saved_file": str(output_path)}, str(output_path), None
        payload = result.body if isinstance(result.body, dict) else {}
        metrics = payload.get("metrics") if isinstance(payload, dict) else None
        if metrics is not None:
            metrics = localize_metrics(metrics)
...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="Local WaifuDiffusion Tagger") as demo:
        ...
        submit_single.click(
            ...
            outputs=[single_text, single_json, single_file, single_metrics],
        )
        submit_batch.click(
